...
export default function ClipsProductApp() {
  return (
...
const ClipsProductPage: React.FC = () => {
  const [quantity, setQuantity] = useState(1);
  const [loading, setLoading] = useState(false);
  const [added, setAdded] = useState(false);
  const [mainImage, setMainImage] = useState('/images/LNL clips.jpg');

  const product = { name: 'Installation Clips', price: 19.99, originalPrice: 29.99 };

  const thumbnails = [
    { src: '/images/LNL clips.jpg', alt: 'Installation clips' },
  ];
  ...
  const features = [
    {
      ...
      description: 'Built to withstand harsh weather conditions and secure your lights firmly in place.'
    },
    {
      icon: <Zap className="w-8 h-8 text-teal-600" />,
      title: 'Universal Compatibility',
      description: 'Works with all Linkin Lights products and most standard string light systems.'
    },
    {
      icon: <Award className="w-8 h-8 text-teal-600" />,
      title: 'No Surface Damage',
      description: 'Specially designed to secure lights without damaging gutters, shingles, or surfaces.'
    }
  ];
  ...
  const handleAddToCart = async () => {
    setLoading(true);
    try {
      await addToCart('gid://shopify/ProductVariant/123456791', quantity);
      setAdded(true);
    ...
    <main className="container mx-auto px-4 sm:px-6 lg:px-8 py-8 md:py-12 pt-24 bg-gray-50 min-h-screen">
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 md:gap-12">
        ...
        <div className="space-y-6">
          <h1 className="text-3xl md:text-4xl font-bold text-gray-800">OUTDOOR CLIPS</h1>
          
          <p className="text-gray-600 text-lg">
            Adhesive String Light Clips – Sticky Cord Hooks for Outdoor Lights, Holiday Displays, and Cable Management
          </p>
      ...
      <section className="py-12 md:py-20">
        <div className="text-center mb-12">
          <h2 className="text-3xl md:text-4xl font-bold text-gray-800">Easy Installation</h2>
          <p className="text-lg text-gray-600 mt-2">Secure your lights in minutes with our professional-grade clips.</p>
        </div>
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 md:gap-12 items-center">
          <div className="relative">
            <img
              src="/images/LNL clips.jpg"
              alt="Installation clips demonstration"
              className="w-full h-auto rounded-lg shadow-xl"
            />
          </div>
          <div className="space-y-6">
            {[
              { step: 1, title: 'Position the clip', description: 'Place the clip on your gutter, shingle, or desired surface.' },
              { step: 2, title: 'Insert your lights', description: 'Slide your Linkin Lights string into the secure grip channel.' },
              { step: 3, title: 'Press to secure', description: 'Apply gentle pressure to lock the clip in place - no tools needed!' }
            ].map((item) => (
